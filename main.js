...
// is tied to particular dataset object which is also called 'data join'. Data function creates placeholder references for any elements in dataset that are not existing in initial selection and returns this selection. 
...
// When defining a scale, you define 2 type of ranges: 'domain' (input values) and 'range' (output values) where you define minimum and maximum values for each.
...
// Append method appends named element as last child of current selection and returns collection of these newly created element(s). attr function returns current selection.
